...
* config.[name]/[Project].cmake : A project configuration file (see
  Configuration)
* Build/ : The default build directory where all generated files land (can be set to a different location by running make with Build=<desired build path> argument).
* Build/[Project] : The project build directory, including binaries
* Release/... : Same as build, but for a 'make release' build
* src/ : The default directories into which all project sources are cloned (can be set to a different location by running make with CMakeArgs="-DEXTERNAL_PACKAGES_SOURCE_DIR=BINARY_DIR" argument).
